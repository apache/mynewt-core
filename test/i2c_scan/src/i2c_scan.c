...
static int
i2c_scan_cli_cmd(const struct shell_cmd *cmd, int argc, char **argv,
                 struct streamer *streamer)
{
    ...
    if (argc < 2) {
        streamer_printf(streamer, "Specify i2c num\n");
        return 0;
    }
    ...
    if (rc) {
        streamer_printf(streamer, "Invalid i2c interface:%s\n", argv[2]);
        return 0;
    }

    streamer_printf(streamer, "Scanning I2C bus %u\n"
    ...
    for (addr = 0x08; addr < 0x78; addr++) {
        ...
        if (!rc) {
            streamer_printf(streamer, "%02x ", addr);
            dev_count++;
        } else {
            streamer_printf(streamer, "-- ");
        }
