...
//static mbedtls_ecdh_context ble_l2cap_sm_alg_ecdh_ctx;
//static mbedtls_cmac_context ble_l2cap_sm_alg_cmac_ctxt;

/* based on Core Specification 4.2 Vol 3. Part H 2.3.5.6.1 */
static const uint32_t ble_l2cap_sm_alg_dbg_priv_key[8] = {
    0xcd3c1abd, 0x5899b8a6, 0xeb40b799, 0x4aff607b, 0xd2103f50, 0x74c9b3e3,
    0xa3c55f38, 0x3f49f6d4
};

static const uint8_t ble_l2cap_sm_alg_dbg_pub_key[64] = {
...
static const uint8_t ble_l2cap_sm_alg_dbg_f4[16] = {
    0x2d, 0x87, 0x74, 0xa9, 0xbe, 0xa1, 0xed, 0xf1,
    0x1c, 0xbd, 0xa9, 0x07, 0xf1, 0x16, 0xc9, 0xf2,
};

static const uint8_t ble_l2cap_sm_alg_dbg_f5[32] = {
...
static const uint8_t ble_l2cap_sm_alg_dbg_f6[16] = {
...
#if 0
static void
ble_l2cap_sm_alg_log_buf(char *name, uint8_t *buf, int len)
{
...
int
ble_l2cap_sm_alg_f4(uint8_t *u, uint8_t *v, uint8_t *x, uint8_t z,
                    uint8_t *out_enc_data)
{
#if 0
    uint8_t xs[16];
    ...
    BLE_HS_LOG(DEBUG, "ble_l2cap_sm_alg_f4()\n    u=");
    ...
    mbedtls_cmac_init(&ble_l2cap_sm_alg_cmac_ctxt);
    ...
    rc = mbedtls_aes_cmac_prf_128(&ble_l2cap_sm_alg_cmac_ctxt, xs, sizeof xs,
    ...
    memcpy(out_enc_data, ble_l2cap_sm_alg_dbg_f4,
           sizeof ble_l2cap_sm_alg_dbg_f4);
    return 0;
}

int
ble_l2cap_sm_alg_g2(uint8_t *u, uint8_t *v, uint8_t *x, uint8_t *y,
                    uint32_t *passkey)
{
#if 0
    uint8_t m[80], xs[16];
    ...
    ble_l2cap_sm_alg_log_buf("u", u, 32);
    ble_l2cap_sm_alg_log_buf("v", v, 32);
    ble_l2cap_sm_alg_log_buf("x", x, 16);
    ble_l2cap_sm_alg_log_buf("y", y, 16);
    ...
    ble_l2cap_sm_alg_log_buf("res", xs, 16);
...
int
ble_l2cap_sm_alg_f5(uint8_t *w, uint8_t *n1, uint8_t *n2, uint8_t a1t,
                    ...
                    uint8_t *ltk)
{
    ...
    ble_l2cap_sm_alg_log_buf("w", w, 32);
    ble_l2cap_sm_alg_log_buf("n1", n1, 16);
    ble_l2cap_sm_alg_log_buf("n2", n2, 16);

    mbedtls_cmac_init(&ble_l2cap_sm_alg_cmac_ctxt);

    swap_buf(ws, w, 32);

    rc = mbedtls_aes_cmac_prf_128(&ble_l2cap_sm_alg_cmac_ctxt,
                                  salt, sizeof salt, ws, sizeof ws, t);
    if (rc != 0) {
        return BLE_HS_EUNKNOWN;
    }

    ble_l2cap_sm_alg_log_buf("t", t, 16);
    ...
    ble_l2cap_sm_alg_log_buf("mackey", mackey, 16);
    ...
    ble_l2cap_sm_alg_log_buf("ltk", ltk, 16);

    swap_in_place(ltk, 16);
#endif

    memcpy(mackey, ble_l2cap_sm_alg_dbg_f5 + 16, 16);
    memcpy(ltk, ble_l2cap_sm_alg_dbg_f5, 16);
    return 0;
}

int
ble_l2cap_sm_alg_f6(uint8_t *w, uint8_t *n1, uint8_t *n2, uint8_t *r,
                    ...
                    uint8_t a2t, uint8_t *a2, uint8_t *check)
{
    ...
    ble_l2cap_sm_alg_log_buf("w", w, 16);
    ble_l2cap_sm_alg_log_buf("n1", n1, 16);
    ble_l2cap_sm_alg_log_buf("n2", n2, 16);
    ble_l2cap_sm_alg_log_buf("r", r, 16);
    ble_l2cap_sm_alg_log_buf("iocap", iocap, 3);
    ble_l2cap_sm_alg_log_buf("a1t", &a1t, 1);
    ble_l2cap_sm_alg_log_buf("a1", a1, 6);
    ble_l2cap_sm_alg_log_buf("a2t", &a2t, 1);
    ble_l2cap_sm_alg_log_buf("a2", a2, 6);
    ...
    ble_l2cap_sm_alg_log_buf("res", check, 16);

    swap_in_place(check, 16);
#endif

    memcpy(check, ble_l2cap_sm_alg_dbg_f6, sizeof ble_l2cap_sm_alg_dbg_f6);
    return 0;
}
...
#if 0
static int
ble_l2cap_sm_alg_rnd(void *arg, unsigned char *dst, size_t len)
{
    // XXX
    return 0;
}
 ...
 */
int
ble_l2cap_sm_alg_gen_key_pair(uint8_t *pub, uint8_t *priv)
{
    memcpy(pub, ble_l2cap_sm_alg_dbg_pub_key,
           sizeof ble_l2cap_sm_alg_dbg_pub_key);
    memcpy(priv, ble_l2cap_sm_alg_dbg_priv_key,
           sizeof ble_l2cap_sm_alg_dbg_priv_key);
