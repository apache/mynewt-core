...
#define BLINK_FLG_SCHEDULE_ON			0x02
#define BLINK_FLG_SCHEDULE_OFF			0x04
 ...
 * @param on			pointer on time delay for 'on' state
 * @param off			pointer on time delay for 'off' state
 *
...
blink_schedule_next_onoff(blink_t *blink) 
{
	...
	blink->running.flags &= ~(BLINK_FLG_SCHEDULE_ON |
				  BLINK_FLG_SCHEDULE_OFF);

	/* Infinite on */
	if        (b_on  == BLINK_TIME_FOREVER) {
	    blink->running.flags |= BLINK_FLG_SCHEDULE_ON;
	    os_callout_reset(&blink->on_callout, 0);
	    os_callout_stop(&blink->off_callout);
	    
	/* Infinite off */
	} else if (b_off == BLINK_TIME_FOREVER) {
	    blink->running.flags |= BLINK_FLG_SCHEDULE_OFF;
	    os_callout_stop(&blink->on_callout);
	    os_callout_reset(&blink->off_callout, 0);
	...
	callouts:
	    blink->running.flags |= BLINK_FLG_SCHEDULE_OFF;
	    os_callout_reset(&blink->on_callout,  delay);
	    os_callout_reset(&blink->off_callout, delay +
			                          b_on * blink->time_unit);
	}
    /* No more blink */
    } else {
...
/*
 * Process 'on' blink event.
 *
 * @param ev			generated 'on' event
 */
static void
blink_on_callout_handler(struct os_event *ev)
{
    blink_t *blink;
    blink = (blink_t *)ev->ev_arg;
    blink->set_state(true);

    os_mutex_pend(&blink->mutex, OS_WAIT_FOREVER);
    if (blink->running.flags & BLINK_FLG_SCHEDULE_ON) {
	blink_schedule_next_onoff(blink);
    }
...
/*
 * Process 'off' blink event.
 *
 * @param ev			generated 'off' event
 */
static void
blink_off_callout_handler(struct os_event *ev)
{
    blink_t *blink;
    blink = (blink_t *)ev->ev_arg;
    ...
    os_mutex_pend(&blink->mutex, OS_WAIT_FOREVER);
    if (blink->running.flags & BLINK_FLG_SCHEDULE_OFF) {
	blink_schedule_next_onoff(blink);
    }
    os_mutex_release(&blink->mutex);
}
...
blink_init(blink_t *blink)
{
    ...
    /* Initialize callout */
    eventq = MYNEWT_VAL(BLINK_EVENTQ);
    os_callout_init(&blink->on_callout,  eventq,
		    blink_on_callout_handler, blink);
    os_callout_init(&blink->off_callout, eventq,
		    blink_off_callout_handler, blink);
