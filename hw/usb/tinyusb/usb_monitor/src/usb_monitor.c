...
usb_monitor_cb(struct os_event *ev)
{
    ...
    os_callout_reset(&g_ums.monitor_callout,
                     os_time_ms_to_ticks32(MYNEWT_VAL(USB_MONITOR_RATE_MS)));
}

static void
usb_monitor_evq_set(struct os_eventq *evq)
{
    os_callout_init(&g_ums.monitor_callout, evq, usb_monitor_cb, NULL);
}

int
usb_monitor_init(void)
{
    /* Initialize state */
    memset(&g_ums, 0, sizeof(g_ums));

#if MYNEWT_VAL_USB_EVQ
    /* Set event queue if specified */
    usb_monitor_evq_set(MYNEWT_VAL(USB_EVQ));
#else
    /* Use default event queue */
    usb_monitor_evq_set(os_eventq_dflt_get());
#endif
...
void
usb_monitor_register_cb(usb_monitor_cb_t cb)
{
    g_ums.callback = cb;
}

bool
usb_monitor_is_connected(void)
{
    return g_ums.last_connected_state;
}
#endif /* MYNEWT_VAL(USB_MONITOR) */
