...
#ifdef MYNEWT_VAL_USB_MONITOR_EVQ
struct os_eventq MYNEWT_VAL(USB_MONITOR_EVQ);
#endif
...
#if MYNEWT_VAL(USB_MONITOR)
static void
da1469x_usb_monitor_cb(bool connected)
{
...
tinyusb_hardware_init(void)
{
    ...
    if (MYNEWT_VAL_CHOICE(DA1469X_USB_VBUS_HANDLING, auto)) {
         ...
         */
        tinyusb_vbus_changed(tinyusb_is_connected());
#else
    ...
    usb_monitor_register_cb(da1469x_usb_monitor_cb);
...
/*
 * Public API functions - map to usb_monitor functions
 */
#if MYNEWT_VAL(USB_MONITOR)

void
tinyusb_usb_monitor_register_cb(void (*cb)(bool connected))
{
    usb_monitor_register_cb(cb);
}
...
tinyusb_is_connected(void)
{
    ...
    usb_active = usb_monitor_is_connected();
