...
#define OS_TICK_TIMER       NRF_RADIOCORE_RTC_S
#define OS_TICK_IRQ         RTC_IRQn
...
static void
nrf54h20_rad_timer_handler(void)
{
    int delta;
    int ticks;
    os_sr_t sr;
...
os_tick_init(uint32_t os_ticks_per_sec, int prio)
{
    ...
    NVIC_SetVector(OS_TICK_IRQ, (uint32_t)nrf54h20_rad_timer_handler);
