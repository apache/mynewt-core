...
static void
nrf54h20_rad_hal_wdt_default_handler(void)
{
    assert(0);
}
...
static void
nrf54h20_rad_wdt_irq_handler(void)
{
    os_trace_isr_enter();
    if (NRF_WDT->INTENSET & WDT_INTENSET_TIMEOUT_Msk) {
        NRF_WDT->EVENTS_TIMEOUT = 0;
        nrf54h20_rad_hal_wdt_default_handler();
    }
    os_trace_isr_exit();
}
...
hal_watchdog_init(uint32_t expire_msecs)
{
    ...
    NVIC_SetVector(WDT_IRQn, (uint32_t)nrf54h20_rad_wdt_irq_handler);
