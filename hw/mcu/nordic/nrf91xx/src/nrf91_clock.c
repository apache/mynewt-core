...
nrf91_clock_hfxo_request(void)
{
    ...
    assert(nrf91_clock_hfxo_refcnt < 0xff);
    if (nrf91_clock_hfxo_refcnt == 0) {
        NRF_CLOCK->TASKS_HFCLKSTART = 1;
        started = 1;
    }
    ++nrf91_clock_hfxo_refcnt;
...
nrf91_clock_hfxo_release(void)
{
    ...
    stopped = 0;
    __HAL_DISABLE_INTERRUPTS(ctx);
    assert(nrf91_clock_hfxo_refcnt != 0);
    --nrf91_clock_hfxo_refcnt;
    if (nrf91_clock_hfxo_refcnt == 0) {
