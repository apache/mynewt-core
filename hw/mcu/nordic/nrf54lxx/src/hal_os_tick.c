...
#define OS_TICK_TIMER       NRF_RTC10_S
#define OS_TICK_IRQ         RTC10_IRQn
...
static inline void
nrf54l_os_tick_set_ocmp(uint32_t ocmp)
{
    int delta;
    uint32_t counter;
...
nrf54l_timer_handler(void)
{
    ...
    nrf54l_os_tick_set_ocmp(g_hal_os_tick.lastocmp + g_hal_os_tick.ticks_per_ostick);
...
os_tick_init(uint32_t os_ticks_per_sec, int prio)
{
    ...
    NVIC_SetVector(OS_TICK_IRQ, (uint32_t)nrf54l_timer_handler);
