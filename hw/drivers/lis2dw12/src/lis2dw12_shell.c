...
//todo duplicating sensor version
static char*
lis2dw12_ftostr(float num, char *fltstr, int len)
{
    memset(fltstr, 0, len);
...
lis2dw12_shell_cmd_read(int argc, char **argv)
{
    ...
    while(samples--) {
        ...
        console_printf("x:%s ", lis2dw12_ftostr(fx, tmpstr, 13));
        console_printf("y:%s ", lis2dw12_ftostr(fy, tmpstr, 13));
        console_printf("z:%s\n", lis2dw12_ftostr(fz, tmpstr, 13));
    }
