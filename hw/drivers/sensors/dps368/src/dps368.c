...
dps368_set_mode(struct dps368 *dps368, dps3xx_operating_modes_e mode)
{
	...
	os_time_delay((OS_TICKS_PER_SEC * IFX_DPS368_CONFIG_TIME_MS*10)/1000+1);
...
dps368_validate_sample_accuracy(struct dps368 *dps368)
{
	...
	if ((rc=dps368_read_regs(itf,IFX_DPS368_MEAS_CFG_REG_ADDR,
			&read_buffer,IFX_DPS368_MEAS_CFG_REG_LEN))){
		return rc;
	}
...
dps368_get_temperature_degC(struct dps368 *dps368,float *tempdegC)
{
	...
	rc = dps368_read_regs(itf,IFX_DPS368_PSR_TMP_READ_REG_ADDR+3,read_buffer,
...
int dps368_soft_reset(struct sensor *sensor)
{
	...
	rc = dps368_write_reg(itf,IFX_DPS368_SOFT_RESET_REG_ADDR,IFX_DPS368_SOFT_RESET_REG_DATA);
...
INIT_SEQUENCE:
	...
	if (ready == 1){
		...
		if((rc=dps368_prepare_calib_coeff(itf,&dps368->calib_coeffs,&dps368->temp_src))){
			return rc;
		}
...
RECONF:
	...
	if((rc=dps368_reconfig(dps368,updated_cfg.osr_t,updated_cfg.osr_p,updated_cfg.odr_t,updated_cfg.odr_p))){
			return rc;
		}

	DPS368_LOG(INFO,"DPS368:Reconfig done\n");

	if(cfg->config_opt & DPS3xx_RECONF_ALL)
		goto RECONF_MODE_ONLY;
	else
		return 0;
...
RECONF_MODE_ONLY:
	...
	if((rc=dps368_set_mode(dps368,cfg->mode))){
			return rc;
	}
	DPS368_LOG(INFO,"DPS368:Mode set\n");
...
/*Local call-backs implementation*/
		...
		sensor_data_func_t data_func, void *data_arg, uint32_t timeout)
{
	...
	if (dps368->mode == DPS3xx_MODE_IDLE) {
		...
		DPS368_LOG(ERROR,"Could not stream as mode is inappropriate\n "
	...
	if (type & SENSOR_TYPE_TEMPERATURE) {
		...
		DPS368_LOG(INFO,"TEMP is valid:%d\n",std.std_temp_is_valid);
...
/*Bus- peripheral comm based instantiation */
#if MYNEWT_VAL(BUS_DRIVER_PRESENT)
static void
