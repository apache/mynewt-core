                    ...
                    const void *body, int body_len)
{
    ...
    if (hdr->ue_flags & LOG_FLAGS_TRAILER_SUPPORT &&
         ...
         * ------------------------------------------------------
         */
        rc = log_fcb_start_append(log, hdr_len + body_len +
                                  log->l_tr_len + trailer_alignment +
                                  LOG_TRAILER_LEN_SIZE, &loc);
    } else
#endif
    {
    ...
    if (hdr->ue_flags & LOG_FLAGS_TRAILER_SUPPORT) {
        if (log->l_tr_len && log->l_tr_om) {
            ...
            rc = os_mbuf_copydata(log->l_tr_om, 0, log->l_tr_len, buf + offset);
            if (rc) {
                return rc;
            }
            offset += log->l_tr_len;
            memcpy(buf + offset, &log->l_tr_len, LOG_TRAILER_LEN_SIZE);
                         ...
                         struct os_mbuf *om)
{
    ...
    if (hdr->ue_flags & LOG_FLAGS_TRAILER_SUPPORT) {
        ...
        if (log->l_tr_len && log->l_tr_om) {
            ...
            trailer_alignment = log_fcb_hdr_trailer_bytes(fcb->f_align,
                                                          os_mbuf_len(om));
            len += (trailer_alignment + log->l_tr_len + LOG_TRAILER_LEN_SIZE);
        }
    }
#endif
    ...
    if (hdr->ue_flags & LOG_FLAGS_TRAILER_SUPPORT) {
        ...
        if (log->l_tr_len && log->l_tr_om) {
            ...
            rc = os_mbuf_appendfrom(om, log->l_tr_om, 0, os_mbuf_len(log->l_tr_om));
            if (rc) {
                return rc;
            }
            ...
            rc = os_mbuf_copyinto(om, os_mbuf_len(om), &log->l_tr_len,
