...
#define SHELL_MODULE_STD(_name, _command_begin, _command_end) \
    const struct shell_mod_std shell_module_ ## _name = {     \
        { &shell_mod_std_ops, #_name },                       \
        _command_begin, _command_end };                       \
    const struct shell_mod_std *const shell_module_elem_ ## _name SHELL_MODULES_SECTION = &shell_module_ ## _name;
 ...
 */
#define SHELL_MODULE_WITH_TABLE(_name, _table) SHELL_MODULE_STD(_name, _table, _table + ARRAY_SIZE(_table))

/* Section name for shell compat (global) commands */
#define SHELL_COMPAT_CMDS_SECTION __attribute__((section(".shell_compat_cmds"), used))
...
#define SHELL_MODULE_WITH_LINK_TABLE(_name) \
    ...
    SHELL_MODULE_STD(_name, (const struct shell_cmd *)LINK_TABLE_START(_name ## _commands), \
                     (const struct shell_cmd *)LINK_TABLE_END(_name ## _commands))
 ...
 */
#define SHELL_MODULE_CMD_WITH_NAME(mod_, cmd_, name_, func_, help_) \
    const struct shell_cmd shell_cmd_ ## cmd_ LINK_TABLE_ELEMENT_SECTION(mod_ ## _commands, cmd_) = { \
        .sc_ext = 1,                                \
...
#define SHELL_MODULE_CMD(mod_, cmd_, func_, help_) \
    SHELL_MODULE_CMD_WITH_NAME(mod_, cmd_, #cmd_, func_, help_)
