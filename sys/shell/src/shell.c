...
const struct shell_mod_std *const shell_module_elem_compat SHELL_MODULES_SECTION = &shell_mod_compat;
 ...
 */
static const char *
get_command_and_module(char *argv[], const struct shell_mod **module, struct streamer *streamer)
{
...
static void
print_command_params(const struct shell_cmd *shell_cmd,
...
{
    FOR_LINK_TABLE(mod, shell_modules) {
        streamer_printf(streamer, "%s\n", (*mod)->name);
    }
}
...
shell_exec(int argc, char **argv, struct streamer *streamer)
{
     ...
     */
    if (def_module == NULL && cmd != &shell_cmd_select &&
        cmd != &shell_cmd_help) {
        argc_offset = 1;
    }
...
complete_param(char *line, const char *param_prefix,
               int param_len, const struct shell_cmd *command,
...
complete_command(char *line, char *command_prefix,
                 int command_len, const struct shell_mod *module,
                 console_append_char_cb append_char)
{
    int first_match = -1;
    int match_count = 0;
