                                       ...
                                       size_t *num_regions)
{
    ...
    s_coredump_regions[0].region_size = mem->hbmd_size;
    s_coredump_regions[0].region_start = mem->hbmd_start;
    s_coredump_regions[0].type = kMfltCoredumpRegionType_Memory;
...
prv_flash_open(const struct flash_area **fa)
{
    ...
    if (flash_area_open(MYNEWT_VAL(MEMFAULT_COREDUMP_FLASH_AREA), fa)) {
        return -OS_ERROR;
    }
     ...
     * confirmed).
     */
    slot = flash_area_id_to_image_slot(MYNEWT_VAL(MEMFAULT_COREDUMP_FLASH_AREA));
